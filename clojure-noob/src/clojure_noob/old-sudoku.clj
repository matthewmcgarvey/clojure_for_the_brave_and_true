...
(defn value [[_ {value :value}]] value)
...
(defn horizontal-rows [sudoku]
  (vals (group-by (fn [[name _]] (take 1 name)) sudoku)))

(defn vertical-rows [sudoku]
  (vals (group-by (fn [[name _]] (take 1 (str/reverse name))) sudoku)))
